...
var webFXTreeConfig = {		
	rootIcon        : this.imagePath + '/rooticon.png',
	openRootIcon    : this.imagePath + '/openrooticon.png',
	...
	setCascadeCheck:function(bl){
		if(bl == null) bl = true;
		this.cascadeCheck = bl;
	}
};
...
function WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, sTitle) {
	this.base = WebFXTreeAbstractNode;
	this.base(sText, sAction);
	/* Defaults to close */
	if (webFXTreeConfig.usePersistence) {
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\">" +
		...
		"<img id=\"" + this.id + "-icon\" class=\"webfx-tree-icon\" src=\"" + ((webFXTreeHandler.behavior == 'classic' && this.open)?this.openIcon:this.icon) + "\" onclick=\"webFXTreeHandler.select(this);\">" +
		"<a title=\"" + this.title + "\" href=\"" + this.action + "\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onblur=\"webFXTreeHandler.blur(this);\"" +
		(this.target ? " target=\"" + this.target + "\"" : "") + " style='color:" + this.getColor() + "'" +
		">" + label + "</a></div>" +
		"<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open)?'block':'none') + ";\">";
	var sb = [];
...
//自动展开功能菜单
function autoExpand(){
	$(".webfx-tree-container .webfx-tree-item").each(function(){
