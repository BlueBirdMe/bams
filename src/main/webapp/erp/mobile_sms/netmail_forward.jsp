...
window.onload = function(){
	useLoadingMassage();
	initInput('title','您可以在此处转发邮件！');
	dwrMailService.getNetMailInboxDetailForwardById(<%=mid%>,setSendBox);
...
function savemail(type){
	...
	if(bl){
		if(type==2){//存为草稿
			...
			var empid = DWRUtil.getValue("oaMailSendEmpids");
			if((adder==""||adder.length==0)&&(empid==""||empid.length==0)){
				 setMustWarn("oasendMailAdderMust","收件地址或内部收件人需选填一项。");
				 window.scrollTo(0,0);
				 return false;
			}
			Btn.close(document.getElementById("button_submit"));
...
<div class="formDetail">
	...
	<tr>
		<th width="15%"><span style="color:blue">•</span>&nbsp;收件地址</th>
		<td  colspan="2">
	...
	<tr>
		<th width="15%"><span style="color:blue">•</span>&nbsp;内部收件人</th>
		<td colspan="2">
