...
function reback(index,dialogId){
  ...
  var url = "<%=contextPath%>/erp/mobile_sms/mail_send.jsp?mailtype="+index+"&backtype=3&mailempPk="+mailempPk+"";
  openMDITab(url);
  Sys.close(dialogId);
}

function returnload(){
	Sys.load("<%=contextPath%>/erp/mobile_sms/mail_draftbox.jsp",'mail');
}
