...
function queryData(){
	...
	sendbox.oaNetmailSendType = <%=EnumUtil.Net_Mail_SendBox_Type.Send.value%>;
...
function dblCallback(obj){
	var box = new Sys.msgbox('已发送邮件明细查看','<%=contextPath%>/erp/mobile_sms/netmail_send_detail.jsp?mid='+obj.value+'&sid='+document.getElementById("sendMailSel").value,'800','500');
	box.msgtitle="<b>已发送邮件信息明细查看</b>";
	var butarray = new Array();
	butarray[0] = "ok|sendsign('"+box.dialogId+"');| 重 发 ";
	butarray[1] = "cancel";
...
function sendagain(id){
	var url = '<%=contextPath%>/erp/mobile_sms/netmail_send.jsp?mid='+id+'&type=send';
...
function delbatchok(){
	...
	dwrMailService.deleteSendboxBySendPks(recordsPks,delcallback);
}

function delok(pk){
	var pks = new Array();
	pks[0] = pk;
	dwrMailService.deleteSendboxBySendPks(pks,delcallback);
}
...
<table width="100%" height="100%" cellspacing="0" cellpadding="0" border="0" style="background: url('<%=contextPath %>/images/alertimg/content_bg.gif') repeat-x 0 0;">
	...
	<%
		SysGrid bg = new SysGrid(request);
			...
			sccList.add(new SysColumnControl("oaNetmailSendTitle","邮件主题",1,1,1,30));
			sccList.add(new SysColumnControl("oaNetmailSendTime","发送时间",1,2,1,0));
