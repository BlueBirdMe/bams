...
function queryData(){
   ...
   var pager = getPager();
	...
	dwrMailService.listOaMailSend_draft(oaMailSend,pager,queryCallback);
}
...
function updateMail(id){
   MoveDiv.show('编辑短信','<%=contextPath%>/erp/mobile_sms/mail_send.jsp?mailtype=3&backtype=5&isEdit=1&mailempPk='+id);
}
	

  //双击数据
function dblCallback(obj){
    var box = new Sys.msgbox('明细查看','<%=contextPath%>/erp/mobile_sms/mail_draftbox_detail.jsp?mailoutid='+obj.value,'800','500');
...
function createMethod(rowObj){
 	//str="<a href='javascript:void(0)' title='编辑' onclick=\"updateMail('"+rowObj.primaryKey+"')\"><img src='<%=contextPath%>/images/grid_images/rowedit.png' border='0'/></a>";
...
function reSend(pk){
    var url = "<%=contextPath%>/erp/mobile_sms/mail_send.jsp?mailtype=3&backtype=3&mailempPk="+pk;
    openMDITab(url);
}
...
<%
    ...
    sccList.add(new SysColumnControl("oaMailSendSaveTime","保存时间",1,2,2,0));
