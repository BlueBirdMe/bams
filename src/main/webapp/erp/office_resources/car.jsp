...
//双击数据
	...
	function add(){
		var url = '<%=contextPath%>/erp/office_resources/car_add.jsp';
		openMDITab(url + "?tab="+getMDITab());
	}
	function edit(pk){
	MoveDiv.show('编辑车辆','<%=contextPath%>/erp/office_resources/car_add.jsp?carpk='+pk);
	...
	function delok(pk){
		var pks = new Array();
		pks[0] = pk;
		dwrOfficeResourcesService.deleteCarsByPks(pks,delcallback);
	}
	...
	function delbatchok(){
		...
		dwrOfficeResourcesService.deleteCarsByPks(recordsPks,delcallback);
	}
	...
	if(rowObj.oaCarSta == <%=EnumUtil.OA_CAR_STA.GOOD.value%>){
		...
		str+="&nbsp;&nbsp;<a href='javascript:void(0)' title='已报废不能编辑'><img src='<%=contextPath%>/images/grid_images/rowedit_.png' border='0'/></a>";
		str+="&nbsp;&nbsp;<a href='javascript:void(0)' title='已报废不能维护' ><img src='<%=contextPath%>/images/grid_images/bullet_wrench.png' border='0' style='filter:gray;'/></a>";
		str+="&nbsp;&nbsp;<a href='javascript:void(0)' title='已报废不能删除' ><img src='<%=contextPath%>/images/grid_images/rowdel_.png' border='0'/></a>";
	}
...
function modi(id){
	var url = "<%=contextPath%>/erp/office_resources/car_mainten_add.jsp?pk="+id+'&url=office_resources/car.jsp'; 
	openMDITab(url);
}
...
<%
	...
	btnList.add(new SysGridBtnBean("新增车辆","add()","add.png"));
