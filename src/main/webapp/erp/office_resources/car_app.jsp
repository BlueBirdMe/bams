...
function queryData() {
	...
	var pager = getPager();
	dwrOfficeResourcesService.listCarapp(caruse, pager, queryCallback);
}
...
function dblCallback(obj) {
	//MoveDiv.show('车辆使用明细','<%=contextPath%>/erp/office_resources/car_use_detail.jsp?cid='+obj.value);
...
function update() { //针对一条记录进行操作
	if (getOneRecordArray() != false) {
		var obj = getObjectByPk(getOneRecordArray());
		Sys.load("office_resources/car_use_add.jsp?carusepk='" + obj.primaryKey + "'");
	}
}
function edit(id) {
...
function update(id) {
	MoveDiv.show('编辑车辆使用','<%=contextPath%>/erp/office_resources/car_use_add.jsp?carusepk='+id);
}
function del(pk) {
...
function delok(pk) {
	var pks = new Array();
	pks[0] = pk;
	dwrOfficeResourcesService.deleteCarusesByPks(pks, delcallback);
}
...
function createProcessMethod(rowObj) {
	...
	str += "<a href='javascript:void(0)' title='车辆使用中不能编辑'><img src='<%=contextPath%>/images/grid_images/rowedit_.png' border='0'/></a>";
		str += "&nbsp;&nbsp;<a href='javascript:void(0)' title='车辆使用中不能删除' ><img src='<%=contextPath%>/images/grid_images/rowdel_.png' border='0'/></a>";
}else{
        str += "<a href='javascript:void(0)' title='车辆使用中不能编辑'><img src='<%=contextPath%>/images/grid_images/rowedit_.png' border='0'/></a>";
		str += "&nbsp;&nbsp;<a href='javascript:void(0)' title='车辆使用中不能删除' ><img src='<%=contextPath%>/images/grid_images/rowdel_.png' border='0'/></a>";
}
...
//放入列
    ArrayList<SysColumnControl> sccList = new ArrayList<SysColumnControl>();
     sccList.add(new SysColumnControl("applyEmployee.hrmEmployeeName","用车人",1,1,1,0));
     sccList.add(new SysColumnControl("oaCar.oaCarName","车辆名称",1,1,1,0));
     sccList.add(new SysColumnControl("applyBegindate","用车时间",1,2,1,0));
     sccList.add(new SysColumnControl("applyEnddate","还车时间",1,2,1,0));
     sccList.add(new SysColumnControl("applyNum","用车人数",1,2,2,0));
     sccList.add(new SysColumnControl("oaCarStatus","使用状态",1,2,2,0));
