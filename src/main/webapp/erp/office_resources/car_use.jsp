...
function dblCallback(obj) {
	//MoveDiv.show('车辆使用明细列表', '<%=contextPath%>/erp/office_resources/car_use_detail.jsp?cid=' + obj.value);
	var box = new Sys.msgbox('明细查看', '<%=contextPath%>/erp/office_resources/car_use_detail.jsp?cid=' + obj.value, '750', '500');
...
//	function update() { //针对一条记录进行操作
	...
	//       alert(123);
	//		Sys.load('office_resources/car_use_add.jsp?carusepk='+obj.primaryKey+'&url=office_resources/car_use.jsp');
	//	}
	//}
//	function edit(id) {
...
//	}
	function update(id) {
		MoveDiv.show('编辑车辆使用', "<%=contextPath%>/erp/office_resources/car_use_add.jsp?carusepk=" + id+'&url=office_resources/car_use.jsp');
}
function del(pk) {
...
function delok(pk) {
	var pks = new Array();
	pks[0] = pk;
	dwrOfficeResourcesService.deleteSuperCarusesByPks(pks, delcallback);
}
...
function delbatchok() {
	...
	dwrOfficeResourcesService.deleteSuperCarusesByPks(recordsPks, delcallback);
}
