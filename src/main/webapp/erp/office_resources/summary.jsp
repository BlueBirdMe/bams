...
function dblCallback(obj){
	var box = new Sys.msgbox('明细查看','<%=contextPath%>/erp/office_resources/summary_detail.jsp?sid='+obj.value,'650','450');
		box.msgtitle="<b>会议纪要明细列表</b>";
		box.show();
}

function add(){
	Sys.load('office_resources/summary_add.jsp');
}
...
function update(){
	...
	if(getOneRecordArray() != false){
		...
		Sys.load("office_resources/summary_add.jsp?summarypk='"+obj.primaryKey+"'");
	}
}
...
function edit(pk){
    Sys.load("office_resources/summary_add.jsp?summarypk='"+pk+"'");
}
...
function del(pk){
	...
	function delok(pk){
		var pks = new Array();
		pks[0] = pk;
		dwrOfficeResourcesService.deleteSummarysByPks(pks,delcallback);
	}
	...
	function delbatchok(){
		...
		dwrOfficeResourcesService.deleteSummarysByPks(recordsPks,delcallback);
	}
...
//放入列
    ...
    sccList.add(new SysColumnControl("oaSummaryName","会议名称",1,1,1,20));
    sccList.add(new SysColumnControl("oaSummaryReader","指定读者",2,2,2,0));
    sccList.add(new SysColumnControl("oaSummaryContent","纪要内容",2,2,2,50));
    sccList.add(new SysColumnControl("oaSummaryDate","录入时间",1,1,1,50));
    sccList.add(new SysColumnControl("oaSummaryMeetDate","会议时间",1,2,1,50));
...
for(int i=0;i<colList.size();i++){
	...
	if(bc.isShowAdvanced()||bc.isShowColumn()){
		if("oaSummaryDate".equalsIgnoreCase(bc.getDataName())){
	//高级查询显示
	...
	bc.setColumnStyle("padding-left:15px;color:blue;");
		...
		if("oaSummaryMeetDate".equalsIgnoreCase(bc.getDataName())){
	//高级查询显示
	DateType date = new DateType();
	date.setDefaultDate(UtilWork.getToday());
	bc.setColumnTypeClass(date);
