...
function queryData(){
    startQuery();
    var dxeducate = getQueryParam();//java实体类相对应
    var pager = getPager();
    dwrDxEducateService.listDxEducate(dxeducate,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/rlzy/dxeducate_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/rlzy/dxeducate_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除培训信息吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrDxEducateService.deleteDxEducateByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除培训信息吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的培训信息...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/rlzy/dxeducate_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"培训信息列表");
//放入按钮
