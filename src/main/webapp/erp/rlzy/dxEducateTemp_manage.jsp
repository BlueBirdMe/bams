...
function queryData(){
    startQuery();
    var dxeducatetemp = getQueryParam();//java实体类相对应
    var pager = getPager();
    dwrDxEducateTempService.listDxEducateTemp(dxeducatetemp,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/rlzy/dxEducateTemp_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/rlzy/dxEducateTemp_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除培训审核吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrDxEducateTempService.deleteDxEducateTempByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除培训审核吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的培训审核...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/rlzy/dxEducateTemp_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"培训审核列表");
//放入按钮
...
grid.setHelpList(UtilTool.getGridTitleList(this.getServletContext(), request));
    ...
    sccList.add(new SysColumnControl("experience", "培训经历", 2, 2, 2, 12));
    sccList.add(new SysColumnControl("certificate", "认证证书", 2, 2, 2, 12));
    sccList.add(new SysColumnControl("specialty", "现从事专业", 1, 2, 2, 0));
    sccList.add(new SysColumnControl("date", "现专业开始时间", 1, 2, 2, 0));
    sccList.add(new SysColumnControl("year", "现专业年从事年限", 1, 2, 2, 0));
    sccList.add(new SysColumnControl("pastspecialty", "曾从事专业", 1, 2, 2, 0));
    sccList.add(new SysColumnControl("pastdate", "曾专业开始时间", 1, 2, 2, 0));
    sccList.add(new SysColumnControl("pstyear", "曾专业年从事年限", 1, 2, 2, 0));
