...
function init(index){
	...
	dwrWorkArrangeService.listOaCalenderYear(year,month,day,index,dateback);
}
...
function dateback(data){
	if(data.success == true){
		...
		if(data.resultList.length > 0){
			...
			otr.style.cssText="color:black;background-color:#fff;text-align: center;line-height:22px;";
			otr.id = calbean.time;
			...
			DWRUtil.setValue("newdate",calbean.year);
...
function addCalender(index){
	...
	datetime = DWRUtil.getValue("year")+"-"+datetime.substring(0,2)+"-01 "+'<%=UtilWork.getNowTime().substring(11, UtilWork.getNowTime().length())%>';
...
function setdate(){
	...
	DWRUtil.setValue("newdate",year);
	dwrWorkArrangeService.listOaCalenderYear(year,month,day,0,dateback);
}
...
<body style="overflow: hidden">
	<div style="display:none;" id='functiondiv'>
		<ul>
			<li id='info'><img src="<%=contextPath%>/images/grid_images/info.png"> <label id="label"></label>
			</li>
		...
		<tr style="width: 100%">
			<td style="padding-bottom: 5px; padding-top: 5px;">
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td align='left' style='padding-left: 15px;' width="20px"><a title='上一年' onclick='init(-1)' href="javascript:void(0)" style="cursor: pointer; font: 12px '宋体'; text-decoration: none;">&nbsp;<<&nbsp;</a></td>
						<td id="newdate" align="center" width="55px"></td>
						<td align='left' width="20px"><a title='下一年' onclick='init(1)' href="javascript:void(0)" style="cursor: pointer; font: 12px '宋体'; text-decoration: none;">&nbsp;>>&nbsp;</a></td>
						<td style='padding-left: 30px;'><btn:btn onclick="setdate()" value=" 今 年 " width="50px"></btn:btn></td>
		...
		<tr style="height: 100%">
			<td valign="top">
				<div style="overflow: auto; height: 100%; border-top: 1px outset #ffffff; width: 100%">
					<sys:table tableTitle="年列表" width="100%" id="resultTable">
