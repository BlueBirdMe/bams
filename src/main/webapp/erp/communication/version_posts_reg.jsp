...
function queryRegPostback(data){
    if(data.success == true){
		if(data.resultList.length > 0){
		     ...
		     titleDIV.innerHTML = "<a class=ForumA href='versionManager_total.jsp'>论坛</a><FONT class=MenuPoint> => </FONT><a class=ForumA href='versionposts_manager.jsp?forumId=<%=forumId %>'>"+data.resultList[0].forums.oaForumName+"</a>";
		     ...
		     for ( var i = 0; i < data.resultList.length; i++) {
		        ...
		        /******************************************************************/  
		          var td1 = document.createElement("td");
		              td1.width= "15%";
		              td1.style.borderBottom = "1px solid #999999";
                 ...
                 /******************************************************************/
		          var td2 = document.createElement("td");
		              ...
		              td2.style.borderBottom = "1px solid #999999";
...
function reload(){
    if(<%=forumId%> != null){
         var  forid =  <%=forumId%>;
         Sys.load('<%=contextPath%>/erp/communication/versionposts_manager.jsp?forumId='+forid,'commun');
    }else{
         Sys.load('<%=contextPath%>/erp/communication/versionposts_manager.jsp','commun');
    } 
}

function reback(){
		window.location="versionposts_manager.jsp?forumId=<%=forumId%>";
}
