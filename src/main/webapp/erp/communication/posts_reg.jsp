...
window.onload=function(){
    ...
    initInput('helpTitle','您可以在此处发表您的看法，参与讨论。');
}
...
function queryCallback(data){
	if(data.success == true){
		if(data.resultList.length > 0){
		   document.getElementById("poststitle").innerHTML = "标题："+data.resultList[0].oaPostName;
...
function getRegInfo(){
	...
	var forumID = <%=Integer.parseInt(forumid)%>;
	var regPostsInfo = new Object();
	regPostsInfo.oaPostReg = postspk;
	regPostsInfo.oaPostForum = forumID;
...
function queryRegPostback(data){
   if(data.success == true){
		if(data.resultList.length > 0){
		     ...
		     titleDIV.innerHTML="<a class=ForumA href='communications_total.jsp'>论坛讨论</a><FONT class=MenuPoint> => </FONT><a class=ForumA href='posts_list.jsp?forumId="
		          					+<%=forumid %>+"'>"+data.resultList[0].forums.oaForumName+"</a>";
		     ...
		     for ( var i = 0; i < data.resultList.length; i++) {
		        ...
		        /******************************************************************/  
		          var td1 = document.createElement("td");
		              td1.width= "15%";
		              td1.style.borderBottom = "1px dotted #cccccc";
                 ...
                 /******************************************************************/
		          var td2 = document.createElement("td");
		              ...
		              td2.style.borderBottom = "1px dotted #cccccc";
...
function reload(){
    if(<%=forumid%> != null){
        var id = <%=forumid%>;
        Sys.load('<%=contextPath%>/erp/communication/posts_list.jsp?forumId='+id,'commun');
    }else{
        Sys.load('<%=contextPath%>/erp/communication/posts_list.jsp','commun');
    }
}

function reback(){
		window.location="posts_list.jsp?forumId=<%=forumid%>";
}
function closePage(){
		closeMDITab();
}
    ...
    <table align="center">
	    <tr>
			<td><btn:btn onclick="saveReg()" value="回 复 " imgsrc="../../images/png-1718.png" title="回复"></btn:btn></td>
