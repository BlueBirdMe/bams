...
function dblCallback(obj){
	//MoveDiv.show('查看投票结果','<%=contextPath%>/erp/communication/vote_view_pager.jsp?voteId='+obj.value);
...
function replaceViewType(rowObj){
	var str="";
	if(rowObj.oaViewType ==<%=EnumUtil.OA_VOTE_VIEW_TYPE.AFTER.value%>){
		str= "<%=EnumUtil.OA_VOTE_VIEW_TYPE.valueOf(EnumUtil.OA_VOTE_VIEW_TYPE.AFTER.value)%>"
	}else if(rowObj.oaViewType ==<%=EnumUtil.OA_VOTE_VIEW_TYPE.LAST.value%>){
		str= "<%=EnumUtil.OA_VOTE_VIEW_TYPE.valueOf(EnumUtil.OA_VOTE_VIEW_TYPE.LAST.value)%>"
	}else if(rowObj.oaViewType ==<%=EnumUtil.OA_VOTE_VIEW_TYPE.PREVIOUS.value%>){
	    str="<%=EnumUtil.OA_VOTE_VIEW_TYPE.valueOf(EnumUtil.OA_VOTE_VIEW_TYPE.PREVIOUS.value)%>";
	}else{
	    str="<%=EnumUtil.OA_VOTE_VIEW_TYPE.valueOf(EnumUtil.OA_VOTE_VIEW_TYPE.NO.value)%>";
	}
	return str;
}
...
function createProcessMethod(rowObj){
	...
	if(rowObj.oaVoteStatus ==<%=EnumUtil.OA_VOTE_STATUS.VOTING.value%>){
		dtr= "<a href='javascript:void(0)' title='立即终止' onclick=\"setVoteEnd('"+rowObj.primaryKey+"')\"><img src='<%=contextPath%>/images/grid_images/valid1_.png' border='0'/></a>&nbsp;&nbsp";
	}else if(rowObj.oaVoteStatus ==<%=EnumUtil.OA_VOTE_STATUS.END.value%>){
		dtr="<a href='javascript:void(0)' title='重新启用' onclick=\"setVoteReused('"+rowObj.primaryKey+"')\"><img src='<%=contextPath%>/images/grid_images/valid1.png' border='0'/></a>&nbsp;&nbsp";
	}else{
	...
	var str ="";
	if(rowObj.oaVoteStatus  == <%=EnumUtil.OA_VOTE_STATUS.VOTING.value%>){
	    str = "<a href='javascript:void(0)' title='无法编辑' ><img src='<%=contextPath%>/images/grid_images/rowedit_.png' border='0'/></a>&nbsp;"
	}else if(rowObj.oaVoteStatus  == <%=EnumUtil.OA_VOTE_STATUS.END.value%>){
	    str = "<a href='javascript:void(0)' title='无法编辑' ><img src='<%=contextPath%>/images/grid_images/rowedit_.png' border='0'/></a>&nbsp;"
	}else{
	...
	var xtr = "";
	if(rowObj.oaVoteStatus  == <%=EnumUtil.OA_VOTE_STATUS.VOTING.value%>){
	    xtr = "<a href='javascript:void(0)' title='无法删除' ><img src='<%=contextPath%>/images/grid_images/rowdel_.png' border='0'/></a>"
	}else{
...
<%
	...
	if ("oaViewType".equalsIgnoreCase(bc.getDataName())) {
		//设置高级查询显示样式

		SelectType select = new SelectType(EnumUtil.OA_VOTE_VIEW_TYPE.getSelectAndText("-1,-请选择查看类型-"));
		select.setCustomerFunction(new String[] { "onchange=\"queryData();\"" });
		bc.setColumnTypeClass(select);

		bc.setColumnReplace("replaceViewType");
		//设置列显示样式
		bc.setColumnStyle("text-align:center;");
	}
