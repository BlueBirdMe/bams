...
//查询方法
	...
	function dblCallback(obj){
		window.location.href = "<%=contextPath%>/erp/company_resources/photos_albumdetail.jsp?aid="+obj.value;
	}
	function set(){
		if(getAllRecordArray() != false){
			window.location.href = "<%=contextPath%>/erp/company_resources/photos_albumset_set.jsp?aid="+getAllRecordArray();
		}else{
			alertmsg("请选择要设置的相册...");
		}
	}
	function openphoto(){
		if(getAllRecordArray() != false){
			window.location.href = "<%=contextPath%>/erp/company_resources/photos_albumdetail.jsp?aid="+getAllRecordArray();
		}else{
			alertmsg("请选择要打开的相册...");
		}
	}
...
<%
	...
	btnList.add(new SysGridBtnBean("设置","set()","set.png"));
