...
function queryData(){
    startQuery();
    var contractType = getQueryParam();//java实体类相对应
    var pager = getPager();
    dwrHrmContractService.listHrmContractType(contractType,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/hrm/contract_type_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/hrm/contract_type_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除合同类型吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrHrmContractService.deleteHrmContractTypeByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除合同类型吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的合同类型...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/hrm/contract_type_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"合同类型列表");
//放入按钮
...
sccList.add(new SysColumnControl("typeName","类别名称",1,1,1,0));
//sccList.add(new SysColumnControl("typeFile","合同模板",1,2,2,0));
sccList.add(new SysColumnControl("typeDesc","类别描述",1,2,2,0));
