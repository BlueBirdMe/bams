...
function dblCallback(obj){
	//MoveDiv.show('记事明细查看','<%=contextPath%>/erp/issue_info/adversaria_detail.jsp?did='+obj.value);
	var box = new Sys.msgbox('明细查看','<%=contextPath%>/erp/issue_info/adversaria_detail.jsp?did='+obj.value,'800','500');
		box.msgtitle="<b>查看公司记事明细</b>";
		box.show();
}
...
function edit(pk){
	MoveDiv.show('编辑记事','<%=contextPath%>/erp/issue_info/adversaria_add.jsp?adversariapk='+pk);
}
...
<%
	...
	if("oaAdverStatus".equalsIgnoreCase(bc.getDataName())){
		...
		SelectType select  = new SelectType(EnumUtil.OA_ISSUEINFO_STATUS.getSelectAndText("-1,-请选择事记状态-"));
		select.setCustomerFunction(new String[]{"onchange=\"queryData();\""});
		bc.setColumnTypeClass(select);
